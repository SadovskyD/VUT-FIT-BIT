...
void List_Init( List *list ) {
	//nastavíme ukazatele prvního a aktivního elementu na NULL
...
void List_Dispose( List *list ) {
	ListElementPtr Next;
	//Musíme projít seznamem a uvolnit allokovanou paměť
...
void List_InsertFirst( List *list, int data ) {
	//allokace mista pro nový prvek
	ListElementPtr newElemPtr = (ListElementPtr) malloc(sizeof(struct ListElement));
	//ošetření chyb mallocu
	if (newElemPtr == NULL){
		List_Error();
	}
	else{
		//nastavíme data v prvku
		newElemPtr->data = data;
		//nastavíme ukazatel na prvni prvek
		newElemPtr->nextElement = list->firstElement;
		//přidáme vytvořený prvek do seznamu
...
void List_DeleteFirst( List *list ) {
	...
	//Pokud byl seznam list prázdný, nic se neděje.
	if (list->firstElement != NULL){
		//musíme si zapamatovat adresu následujícího prvku
		Next=list->firstElement->nextElement;
		//Pokud byl rušený prvek aktivní, aktivita seznamu se ztrácí.
...
void List_DeleteAfter( List *list ) {
	ListElementPtr Replace;
	//Pokud není seznam list aktivní nebo pokud je aktivní poslední prvek seznamu list, nic se neděje.
	if (list->activeElement != NULL && list->activeElement->nextElement != NULL){
		//musíme si zapamatovat adresu následujícího prvku
		Replace = list->activeElement->nextElement->nextElement;
		free(list->activeElement->nextElement);
		//upravime ukazatele
		list->activeElement->nextElement=Replace;
	}
}
...
void List_Next( List *list ) {
	...
	if (list->firstElement != NULL && list->activeElement != NULL){
		...
		//pokud další element už není tak aktivita se ztratí
