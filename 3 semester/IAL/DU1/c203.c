...
void Queue_Init( Queue *queue ) {
	if (queue != NULL){
		//proházíme přes pole a všechny hodnoty v poli queue->array nastavime na '*'
		for(int i=0; i < QUEUE_SIZE;i++){
			queue->array[i]='*';
		}
		//index na začátek a prvního volného místa fronty nastavte na 0
		queue->firstIndex=0;
		queue->freeIndex=0;
	}
	else{
...
void Queue_Remove( Queue *queue ) {
	...
	if(Queue_IsEmpty(queue)){
		...
		//přesuneme ukazatel na první prvek o jeden element dopředu
...
void Queue_Dequeue( Queue *queue, char *dataPtr ) {
	...
	if(Queue_IsEmpty(queue)){
		...
		//vrátí znak ze začátku fronty
		Queue_Front(queue,dataPtr);
		//Odstraní znak ze začátku fronty
		Queue_Remove(queue);
	}
}
...
void Queue_Enqueue( Queue *queue, char data ) {
	...
	if(Queue_IsFull(queue)){
		...
		//posouvame freeIndex o 1 dopredu
